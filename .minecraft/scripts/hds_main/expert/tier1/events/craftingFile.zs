...
function getRandomNextWoodComponent(currentItem as IItemStack) as IItemStack {
    // 随机时不用考虑gt木板原地tp
    var currentIndex = -1;
    ...
    if (currentIndex == -1 || currentIndex == woodComponents.length - 1) {
        return null;
    }
    
    val randomIndex = Math.min(currentIndex + 1 + Math.floor(Math.random() * (woodComponents.length - currentIndex - 1)), woodComponents.length - 1);
    return woodComponents[randomIndex].firstItem;
}
...
events.onPlayerTick(function(event as PlayerTickEvent) {
    ...
    if (!isNull(player.data.isHolding) && playerData.isHolding.asBool()) {
        ...
        if (!isNull(mainHand) && !isNull(offHand) && 
            ...
            val nextComponent = getRandomNextWoodComponent(mainHand);
